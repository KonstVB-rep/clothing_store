...
const CartItem = ({ product }) => {
  ...
  return (
    <div className="w-full border-b-2 border-black border-solid">
      ...
      <div className="grid py-4 gap-6 grid-cols-1 mobile_350:grid-cols-fit_200 min-h-[320px]">
        <Link
          to={`/filter/${type}/${id}`}
          onClick={() => dispatch(getSingleProduct(id))}
        >
          <img
            className="rounded-md w-full h-full object-cover max-h-[450px]"
            src={img}
            alt={name}
          ></img>
        </Link>
        <div className="text-center mobile_350:text-start">
          <p className="text-black text-sm font-inter tracking-normal leading-none pt-2 capitalize">
